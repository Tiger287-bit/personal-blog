...
from .protocol import (
    ...
    validate_heartbeat,
    validate_hello,
    validate_motor_enable,
    validate_motor_set_speed,
    validate_motor_stop,
...
class RosGateway:
    ...
    def __init__(
        ...
        @param node_name     : 握手响应中的App节点名称
        @param max_vx        : vx 绝对值上限
        @param max_vy        : vy 绝对值上限
        @param max_wz        : wz 绝对值上限
        @param max_motor_rpm : 单电机转速绝对值上限，单位RPM
        ...
        self._node_name = node_name or os.getenv(
            "ROS_GATEWAY_NODE_NAME",
            "arduino-app",
        )
        self._limits = {
        ...
        self._max_motor_rpm = self._read_int(
            max_motor_rpm,
            "ROS_GATEWAY_MAX_MOTOR_RPM",
            60,
            1,
            3000,
        )
        ...
        self._motor_enable_callback = None
        self._motor_speed_callback = None
        self._motor_stop_callback = None
    ...
    def on_motor_enable(self, callback):
        """
        @description         : 注册已通过校验的单电机使能或失能回调
        @param callback      : 接收包含enabled字段的命令并返回是否接受的函数
        @return              : 当前RosGateway实例
        """
        self._motor_enable_callback = callback
        return self

    def on_motor_set_speed(self, callback):
        """
        @description         : 注册已通过校验的单电机RPM速度回调
        @param callback      : 接收rpm和acceleration_level字段并返回是否接受的函数
        @return              : 当前RosGateway实例
        """
        self._motor_speed_callback = callback
        return self

    def on_motor_stop(self, callback):
        """
        @description         : 注册ROS 2主动发起的单电机安全停车回调
        @param callback      : 接收规范化停车命令并返回是否接受的函数
        @return              : 当前RosGateway实例
        """
        self._motor_stop_callback = callback
        return self
    ...
    def publish_motor_state(self, state):
        """
        @description         : 校验并发布一台ZDT电机的实时状态
        @param state         : 包含motor_id、speed_rpm、enabled、communication_ok和error的字典
        @return              : 成功入队返回True，无客户端或失败返回False
        """
        required_fields = {
            "motor_id",
            "speed_rpm",
        ...
        missing_fields = required_fields - state.keys()
        if missing_fields:
            raise ValueError(f"motor_state missing fields: {sorted(missing_fields)}")
        if type(state["motor_id"]) is not int or not 1 <= state["motor_id"] <= 255:
            raise ValueError("motor_id must be an integer in range 1-255")
        if type(state["speed_rpm"]) is not int:
            raise ValueError("speed_rpm must be an integer")
        if type(state["enabled"]) is not bool:
            raise ValueError("enabled must be a boolean")
        if type(state["communication_ok"]) is not bool:
            raise ValueError("communication_ok must be a boolean")
        if not isinstance(state["error"], str):
            raise ValueError("error must be a string")
        return self._enqueue("motor_state", **state)
    ...
    def _process_message(self, websocket, raw_message):
        ...
        try:
            ...
            if message_type == "motor_enable":
                with self._state_lock:
                    mode = self._mode
                command = validate_motor_enable(message, mode)
                accepted = self._invoke_command_callback(
                    "motor_enable",
                    self._motor_enable_callback,
                    command,
                )
                with self._state_lock:
                    self._last_sequence = sequence
            ...
            if message_type == "motor_set_speed":
                with self._state_lock:
                    mode = self._mode
                command = validate_motor_set_speed(
                    message,
                    mode,
                    self._max_motor_rpm,
                    self._command_timeout_ms,
            ...
            if message_type == "motor_stop":
                command = validate_motor_stop(message)
                accepted = self._invoke_command_callback(
                    "motor_stop",
    ...
    def _service_timers(self, websocket):
        ...
        if last_cmd > 0.0 and not watchdog_triggered:
            if current - last_cmd > self._command_timeout_ms / 1000.0:
                with self._state_lock:
                    self._watchdog_triggered = True
                self._invoke_stop("motion_command_timeout")
    ...
    @staticmethod
    def _invoke_command_callback(command_name, callback, command):
        """
        @description         : 隔离硬件命令回调异常并统一解释回调接受结果
        @param command_name  : 用于日志和错误报文的命令名称
        @param callback      : 已注册的硬件命令回调
        @param command       : 已完成协议校验的命令字典
        @return              : 回调未显式返回False时返回True
        """
        if callback is None:
            return False
        try:
            return callback(command) is not False
        except Exception as exc:
            print(
                f"[ros_gateway] {command_name} callback failed: "
                f"{type(exc).__name__}: {exc}",
                flush=True,
            )
            raise ProtocolError(
                "command_failed",
                f"{command_name} execution failed: {exc}",
                command.get("seq"),
            ) from exc
